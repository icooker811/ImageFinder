...
<html>
    ...
    <body >
        ...
        <%            sql = "select * from polygon where mapid ='" + request.getParameter("mapid") + "'";
            ...
            int loc_x = 0, loc_y = 0, des_x = 0, des_y = 0;
            while (myresult.next()) {
                ...
                if (j.name.equals(request.getParameter("loc"))) {
                    loc_x = Integer.parseInt(myresult.getString("x")) + (Integer.parseInt(myresult.getString("width")) / 2);
                    loc_y = Integer.parseInt(myresult.getString("y")) + (Integer.parseInt(myresult.getString("height")) / 2);

                }
                if (j.name.equals(request.getParameter("des"))) {
                    des_x = Integer.parseInt(myresult.getString("x")) + (Integer.parseInt(myresult.getString("width")) / 2);
                    des_y = Integer.parseInt(myresult.getString("y")) + (Integer.parseInt(myresult.getString("height")) / 2);
                }
                // if (!myresult.getString("name").equalsIgnoreCase("zone")) {
                //   }
            }
            ...
            //out.println("</table>");
            String sql_loc = "select * from zone order by (POW( ( (x +width/2) - " + loc_x + " ),2) + POW(  ((y+height/2) - " + loc_y + ") ,2 )) LIMIT 1";
            ResultSet set_loc = statement.executeQuery(sql_loc);
            while (set_loc.next()) {

                loczone = set_loc.getString("zoneid");
            }
            String sql_des = "select * from zone order by (POW(  ((x +width/2) - " + des_x + ") ,2 ) + POW(  ((y+height/2) - " + des_y + " ) ,2)) LIMIT 1";
           // out.println(sql_des);
            ResultSet set_des = statement.executeQuery(sql_des);
            while (set_des.next()) {

                deszone = set_des.getString("zoneid");
            }
